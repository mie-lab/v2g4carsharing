...
res = []
for mode in ["irl_2019_sim_prevmode", "irl_2019_sim_noprevmode", "xgb_2019_sim_prevmode", "xgb_2019_sim_noprevmode"]:
    ...
    res_dict = {
        ...
        "duration_wasserstein": float(text[7].split(" ")[-1][:-4]),
        "starttime_wasserstein": float(text[9].split(" ")[-1][:-4]),
        "endtime_wasserstein": float(text[11].split(" ")[-1][:-4]),
        "distance_wasserstein": float(text[13].split(" ")[-1][:-4]),
        "station_zscore": float(text[15].split(" ")[-1][:-4]),
        "mode_share_ratio": float(text[18].split(" ")[-1][:-4]),
    }
    ...
    if "irl" in mode:
        ...
        with open(path, "r") as infile:
            ...
            res_dict["Acc"] = float(text_acc[-2][5:-5])
            res_dict["Bal. Acc"] = float(text_acc[-1][14:-5])
    else:
        ...
        with open(path, "r") as infile:
            ...
            res_dict["Acc"] = acc[0]
            res_dict["Bal. Acc"] = bal_acc[0]
            res_dict["Acc (train)"] = acc[1]
            res_dict["Bal. Acc (train)"] = bal_acc[1]

    res.append(res_dict)
pd.DataFrame(res).to_csv(os.path.join("outputs", "model_comparison.csv"))
